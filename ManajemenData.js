      ...
      try {
        ...
        if (columnsToTrackDs.length > 0) {
          processDataChanges(
            ...
            activeConfig[KUNCI.DS_NAME_HEADER],
            columnsToTrackDs,
...
function formatVmDetail(row, headers, config) {
  ...
  for (const headerName of requiredHeaders) {
    ...
    if (indices[headerName] === -1 && headerName !== K.HEADER_VM_NO_TIKET) {
      throw new Error(
        `Header untuk '${config[headerName]}' (didefinisikan oleh konstanta '${headerName}') tidak ditemukan.`
      );
    }
  }
  ...
  if (activeTickets.length > 0) {
    activeTickets.forEach((ticket) => {
      pesan += `   - <code>${escapeHtml(ticket.id)}</code>: ${escapeHtml(ticket.name)} (${escapeHtml(
        ticket.status
      )})\n`;
    });
  } else {
  ...
  firstRowButtons.push({
    ...
    callback_data: `${K_NOTE.EDIT_ADD}${normalizedPk}`,
  });
  if (vmNote) {
  ...
  try {
    ...
    if (isCallback) {
      ...
      if (userEvent.action === KONSTANTA.PAGINATION_ACTIONS.EXPORT) {
        ...
        kirimPesanTelegram(
          `✅ Permintaan ekspor Anda untuk "<b>${escapeHtml(searchTerm)}</b>" telah ditambahkan ke antrean.`,
          config,
          "HTML",
          null,
    ...
    if (results.length === 0) {
      ...
      isCallback
        ? editMessageText(message, null, userEvent.message.chat.id, userEvent.message.message_id, config)
    ...
    if (results.length === 1 && !isCallback) {
      ...
      kirimPesanTelegram(
        `✅ Ditemukan 1 hasil untuk "<b>${escapeHtml(searchTerm)}</b>":\n\n${pesan}`,
        config,
        "HTML",
        keyboard
      );
...
function handleHistoryInteraction(update, config, userData) {
  ...
  try {
    if (userEvent.action === KONSTANTA.PAGINATION_ACTIONS.EXPORT) {
      ...
      kirimPesanTelegram(
        `✅ Permintaan ekspor Anda untuk "<b>${escapeHtml(title)}</b>" telah ditambahkan ke antrean.`,
        config,
        "HTML",
        null,
        userEvent.message.chat.id
    ...
    if (logsToShow.length === 0) {
      ...
      if (sessionData.pk) {
        message = `ℹ️ <b>Analisis Riwayat:</b>\nTidak ada aktivitas perubahan yang tercatat untuk VM dengan PK: <code>${escapeHtml(
          sessionData.pk
        )}</code>`;
        // Buat tombol kembali
        keyboard = {
          inline_keyboard: [
            [
              {
                text: "⬅️ Kembali ke Detail VM",
                callback_data: `${KONSTANTA.CALLBACK_CEKVM.BACK_TO_DETAIL_PREFIX}${sessionData.pk}`,
              },
            ],
          ],
        };
      ...
      isCallback
        ? editMessageText(message, keyboard, userEvent.message.chat.id, userEvent.message.message_id, config)
        : kirimPesanTelegram(message, config, "HTML", keyboard, userEvent.chat.id);
      return;
    }
    ...
    const formatHistoryEntry = (row) => {
      const timestamp = new Date(row[headerIndices.timestamp]).toLocaleString("id-ID", {
        day: "numeric",
        month: "short",
        year: "numeric",
        hour: "2-digit",
    ...
    overProvisionedDsList.forEach((dsInfo) => {
      ...
      if (migrationPlan.size > 0) {
        migrationPlan.forEach((vms, destDsName) => {
          ...
          vms.forEach((vm) => {
            finalMessage += ` • <code>${escapeHtml(vm.name)}</code> (${vm.provisionedGb.toFixed(2)} GB) | ${escapeHtml(
              vm.criticality
            )} | ${escapeHtml(vm.state)}\n`;
          });
        });
...
function getCombinedLogs(startDate, config) {
  ...
  if (sheetLog && sheetLog.getLastRow() > 1) {
    ...
    if (timestampIndex === -1) {
      throw new Error(
        `Gagal memproses log: Kolom '${timestampHeader}' tidak ditemukan di header sheet 'Log Perubahan'.`
      );
    }
          ...
          if (archiveEndDate >= startDate) {
            ...
            if (archiveFiles.hasNext()) {
              ...
              const relevantLogs = archivedLogs.filter(
                (logObject) => logObject[timestampHeader] && new Date(logObject[timestampHeader]) >= startDate
              );
              const relevantLogsAsArray = relevantLogs.map((logObject) =>
                logHeaders.map((header) => logObject[header] || "")
              );
...
function handleExportRequest(update, config, userData) {
  ...
  let statusMessageId = null;
  try {
    const titleForStatus = exportType
      .replace(/export_|run_export_/, "")
      .replace(/_/g, " ")
      .toUpperCase();
  ...
  try {
    ...
    switch (exportType) {
      ...
      case KONSTANTA.CALLBACK.EXPORT_VC02_VMS: {
        ...
        const allVmData =
          vmSheet.getLastRow() > 1
            ? vmSheet.getRange(2, 1, vmSheet.getLastRow() - 1, vmSheet.getLastColumn()).getValues()
            : [];
        ...
        if (exportType === KONSTANTA.CALLBACK.EXPORT_ALL_VMS) {
          ...
          if (vcenterIndex === -1)
            throw new Error(`Kolom '${vcenterHeaderName}' tidak ditemukan atau salah dikonfigurasi.`);
    ...
    if (data && headers && headers.length > 0) {
      if (data.length > 0) {
        ...
        if (statusMessageId) {
          editMessageText(
            `✅ Proses ekspor untuk <b>${title}</b> telah selesai. Hasilnya telah dikirimkan dalam pesan terpisah.`,
            null,
            chatId,
            statusMessageId,
            config
...
function handlePaginatedVmList(update, config, userData) {
  ...
  try {
    ...
    if (userEvent.action === KONSTANTA.PAGINATION_ACTIONS.EXPORT) {
      ...
      kirimPesanTelegram(
        `✅ Permintaan ekspor Anda untuk VM di <b>${friendlyListType} "${escapeHtml(
          itemName
        )}"</b> telah ditambahkan ke antrean.`,
        config,
        "HTML",
        null,
        chatId
    ...
    if (listType === "cluster") {
      ...
      headerContent += `• <b>Alokasi Resource:</b> ${analysis.totalCpu} vCPU | ${analysis.totalMemory.toFixed(
        0
      )} GB RAM (~${totalMemoryInTb.toFixed(2)} TB)\n`;
      const diskUtilPercent = analysis.diskUtilizationPercent;
      const barLength = 12;
      const filledLength = Math.round((diskUtilPercent / 100) * barLength);
      const emptyLength = barLength - filledLength;
      const progressBar = "█".repeat(filledLength) + "░".repeat(emptyLength);
      headerContent += `• <b>Utilisasi Disk:</b> ${diskUtilPercent.toFixed(
        1
      )}% [<code>${progressBar}</code>] (${analysis.totalVmProvisionedTb.toFixed(
        2
      )} / ${analysis.totalDsCapacityTb.toFixed(2)} TB)\n`;
      ...
      if (details) {
        ...
        headerContent += `• <b>Kapasitas:</b> ${details.capacityGb.toFixed(1)} GB (${details.capacityTb.toFixed(
          2
        )} TB) | <b>Terpakai:</b> ${details.provisionedGb.toFixed(1)} GB (${details.provisionedTb.toFixed(2)} TB)\n`;
        const usage = details.usagePercent;
        const barLength = 12;
        const filledLength = Math.round((usage / 100) * barLength);
        const emptyLength = barLength - filledLength;
    ...
    if (originPk && paginatedView.keyboard) {
      paginatedView.keyboard.inline_keyboard.push([
        { text: `⬅️ Kembali ke Detail VM`, callback_data: `${K_CEKVM.BACK_TO_DETAIL_PREFIX}${originPk}` },
      ]);
    }
...
function executeExportJob(jobData) {
  try {
    ...
    if (jobData.jobType === "history") {
      ...
      searchResults = context.pk ? getVmHistory(context.pk, config) : getCombinedLogs(new Date(0), config); // Ambil semua log jika tidak ada pk
      ...
      // Untuk pekerjaan ekspor non-riwayat
      if (jobData.listType) {
    ...
    if (!results || results.length === 0) {
      kirimPesanTelegram(
        `ℹ️ Tidak ada data untuk diekspor untuk permintaan: "${title}".`,
        config,
        "HTML",
        null,
        chatId
