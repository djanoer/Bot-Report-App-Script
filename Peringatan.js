...
function jalankanPemeriksaanAmbangBatas(config) {
  ...
  try {
      ...
      if (semuaPeringatan.length === 0) {
           return { pesan: "✅  <b>Kondisi Sistem: Aman</b>\n<i>Tidak ada anomali yang terdeteksi pada semua sistem yang dipantau.</i>", keyboard: null };
      }
      
      // 2. Mulai menyusun pesan laporan
      let finalMessage = `🚨 <b>Laporan Kondisi Sistem</b> 🚨\n`;
      finalMessage += `<i>Analisis dijalankan pada: ${new Date().toLocaleString("id-ID", {dateStyle: 'short', timeStyle: 'long'})}</i>\n`;
      ...
      if (vmAlerts.length > 0) {
          ...
          const uptimeByCrit = uptimeAlerts.reduce((acc, alert) => { const crit = alert.kritikalitas || "Lainnya"; acc[crit] = (acc[crit] || 0) + 1; return acc; }, {});
          const matiByCrit = vmMatiAlerts.reduce((acc, alert) => { const crit = alert.kritikalitas || "Lainnya"; acc[crit] = (acc[crit] || 0) + 1; return acc; }, {});
          ...
          if(uptimeAlerts.length > 0){
              finalMessage += `\n• 💡 <b>Uptime > ${config[KONSTANTA.KUNCI_KONFIG.THRESHOLD_VM_UPTIME]} hari:</b> ${uptimeAlerts.length} VM\n`;
              Object.keys(uptimeByCrit).sort(sortCrit).forEach(crit => { finalMessage += `  - ${escapeHtml(crit)}: ${uptimeByCrit[crit]}\n`; });
          }
...
function cekKapasitasDatastore(config, headers, dsData) {
  ...
  if ([dsNameIndex, usedPercentIndex, capacityGbIndex, provisionedGbIndex, capacityTbIndex, provisionedTbIndex].includes(-1)) {
      console.error("Peringatan: Salah satu header penting (Name, Used%, Capacity/Provisioned GB/TB) untuk datastore tidak ditemukan.");
      return [];
  }
  ...
  dsData.forEach((row) => {
      ...
      if (isOverProvisioned || isUsageHigh) {
          ...
          if (isOverProvisioned) {
              ...
              detailLines.push(`   └ ❗️ Over-provisioned by <b>${selisihGb.toFixed(0)} GB</b>. (${totalProvisionedGb.toFixed(0)} GB / ${totalProvisionedTb.toFixed(2)} TB allocated vs ${capacityGb.toFixed(0)} GB / ${capacityTb.toFixed(2)} TB capacity)`);
...
vmData.forEach(row => {
  const criticality = String(row[critIndex] || '').trim().toUpperCase();
