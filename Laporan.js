...
function generateVcenterSummary(config) {
  ...
  const uptimeCategories = { '0_1': 0, '1_2': 0, '2_3': 0, 'invalid': 0 };
  ...
  if (uptimeIndex !== -1) {
    ...
    message += `- Terdata & Valid: <b>${totalUptimeValid} VM</b>, terbagi sbb:\n`;
    message += `    - Di bawah 1 Tahun: ${uptimeCategories['0_1']} VM\n`;
    message += `    - 1 sampai 2 Tahun: ${uptimeCategories['1_2']} VM\n`;
    message += `    - 2 sampai 3 Tahun: ${uptimeCategories['2_3']} VM\n`;
    message += `- Data Tidak Valid/Kosong: <b>${uptimeCategories['invalid']} VM</b>\n`;

    uptimeKeyboard = {
      inline_keyboard: [
        [{ text: `📄 Export < 1 Tahun (${uptimeCategories['0_1']})`, callback_data: 'export_uptime_0_1' }],
        [{ text: `📄 Export 1 - 2 Tahun (${uptimeCategories['1_2']})`, callback_data: 'export_uptime_1_2' }],
        [{ text: `📄 Export 2 - 3 Tahun (${uptimeCategories['2_3']})`, callback_data: 'export_uptime_2_3' }],
        [{ text: `❓ Export Data Tidak Valid (${uptimeCategories['invalid']})`, callback_data: 'export_uptime_invalid' }]
      ]
    };
  }

  return { message: message, keyboard: uptimeKeyboard };
}
*/
function generateVcenterSummary(config) {
    const sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(config[KONSTANTA.KUNCI_KONFIG.SHEET_VM]);
  ...
  function buatLaporanHarianVM() {
    ...
    let pesanLaporan = `<b>Laporan Harian VM - ${startTime.toLocaleDateString('id-ID', { timeZone: "Asia/Jakarta"})}</b>\n`;
    pesanLaporan += `--------------------------------------------------\n`;
    ...
    const expectedLogHeader = ["Timestamp", "Action/Tipe perubahan", KONSTANTA.HEADER_VM.PK, KONSTANTA.HEADER_VM.VM_NAME, "Sheet Name", "Old Value", "New Value", "Detail Perubahan"];
    if (sheetLog.getLastRow() === 0) sheetLog.appendRow(expectedLogHeader);
    ...
    try {
      ...
      if (files.hasNext()) {
        ...
        catch (e) { console.warn(`Gagal parse arsip JSON: ${e.message}`); }
      }
      ...
      let logEntriesToAdd = [];
      let batchNotifications = { baru: [], dihapus: [], dimodifikasi: [] };
      ...
      for (const [id, dataHariIniObj] of mapVmHariIni.entries()) {
        ...
        if (!dataKemarinObj) {
          ...
          batchNotifications.baru.push(logEntry);
        } else if (dataHariIniObj.hash !== dataKemarinObj.hash) {
          for (const key in dataHariIniObj.data) {
            if (key === KONSTANTA.HEADER_VM.UPTIME || key === KONSTANTA.HEADER_VM.PROV_TB) {
              continue; 
            }
            if (Object.prototype.hasOwnProperty.call(dataHariIniObj.data, key) && String(dataHariIniObj.data[key]) !== String(dataKemarinObj.data[key])) {
              ...
              logEntriesToAdd.push(logEntry);
              batchNotifications.dimodifikasi.push(logEntry);
            }
          }
        }
      ...
      for (const [id, dataKemarinObj] of mapVmKemarin.entries()) {
        ...
        logEntriesToAdd.push(logEntry);
        batchNotifications.dihapus.push(logEntry);
      }
      
      let finalButtons = [];
      if (summary.keyboard) {
        finalButtons.push(...summary.keyboard.inline_keyboard);
      }
      ...
      if (logEntriesToAdd.length > 0) {
        ...
        const perubahan = formatBatchNotification(batchNotifications.baru, batchNotifications.dihapus, batchNotifications.dimodifikasi);
        pesanLaporan += perubahan.message;
        if (perubahan.keyboard) {
          finalButtons.push(...perubahan.keyboard.inline_keyboard);
        }
      } else {
        pesanLaporan += "✅ Tidak ada perubahan data yang terdeteksi hari ini.";
      }
  
      let finalKeyboard = null;
      if (finalButtons.length > 0) {
        finalKeyboard = { inline_keyboard: finalButtons };
      }
  
      kirimPesanTelegram(pesanLaporan, config, 'HTML', finalKeyboard);
      ...
      console.log("Pengarsipan selesai.");
