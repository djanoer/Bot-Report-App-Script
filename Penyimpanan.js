...
function processAndLogReport(textBlock, config) {
  ...
  const reportKey = Object.keys(aliasMap).find(key => parsedData.storageName.toLowerCase().includes(key.toLowerCase()));
...
function saveRowToSheet(sheetName, rowData) {
    ...
    if (!sheet) {
        ...
        const headers = [
          "Timestamp", "Storage Name", "Storage Alias", "Usage (TB)", 
          "Total Capacity (TB)", "Snapshot (TB)", "Latency (ms)", 
          "IOPS", "Throughput (MB/s)", "Controller CPU (%)", 
          "Data Reduction Ratio", "Datastore Count"
        ];
...
function convertUnit(value, fromUnit, toUnit) {
    ...
    if (from.startsWith(to) || (from.includes('mib') && to.includes('mb/s')) || (from.includes('gib') && to.includes('gb'))) {
      return val;
    }
