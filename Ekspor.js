...
function exportResultsToSheet(headers, dataRows, title, config, userData, highlightColumnName = null) {
  ...
  /**
   * [PINDAH] Fungsi spesialis untuk menangani semua permintaan ekspor kategori Uptime.
   */
  ...
  /**
   * [REFACTOR STATE-DRIVEN] Mengendalikan semua permintaan ekspor dari menu interaktif.
   * Fungsi ini sekarang membaca tipe ekspor dari data sesi untuk bekerja dengan 'export_machine'.
   */
  function handleExportRequest(update, config, userData) {
    const callbackQuery = update.callback_query;
    const sessionData = callbackQuery.sessionData;
    const exportType = sessionData.type; // Mengambil tipe dari sesi, bukan callbackData langsung
    ...
    try {
      ...
      // Peta untuk mengubah tipe dari sesi ke konstanta lama jika diperlukan oleh fungsi helper
      const exportTypeMap = {
        "log_today": KONSTANTA.CALLBACK.EXPORT_LOG_TODAY,
        "log_7_days": KONSTANTA.CALLBACK.EXPORT_LOG_7_DAYS,
        "log_30_days": KONSTANTA.CALLBACK.EXPORT_LOG_30_DAYS,
        "uptime_cat_1": KONSTANTA.CALLBACK.EXPORT_UPTIME_CAT_1,
        "uptime_cat_2": KONSTANTA.CALLBACK.EXPORT_UPTIME_CAT_2,
        "uptime_cat_3": KONSTANTA.CALLBACK.EXPORT_UPTIME_CAT_3,
        "uptime_cat_4": KONSTANTA.CALLBACK.EXPORT_UPTIME_CAT_4,
        "uptime_invalid": KONSTANTA.CALLBACK.EXPORT_UPTIME_INVALID,
        "all_vms": KONSTANTA.CALLBACK.EXPORT_ALL_VMS,
        "vms_vc01": KONSTANTA.CALLBACK.EXPORT_VC01_VMS,
        "vms_vc02": KONSTANTA.CALLBACK.EXPORT_VC02_VMS,
      };
      
      const internalExportType = exportTypeMap[exportType];
  
      switch (internalExportType) {
        ...
        case KONSTANTA.CALLBACK.EXPORT_LOG_30_DAYS: {
          ...
          if (internalExportType === KONSTANTA.CALLBACK.EXPORT_LOG_TODAY) {
            startDate.setHours(0, 0, 0, 0);
            title = "Log Perubahan Hari Ini (Termasuk Arsip)";
          } else if (internalExportType === KONSTANTA.CALLBACK.EXPORT_LOG_7_DAYS) {
        ...
        case KONSTANTA.CALLBACK.EXPORT_VC02_VMS: {
          ...
          if (internalExportType === KONSTANTA.CALLBACK.EXPORT_ALL_VMS) {
            data = allVmData;
            title = "Semua Data VM";
          } else {
            ...
            const vcenter = internalExportType.split("_").pop().toUpperCase();
        ...
        case KONSTANTA.CALLBACK.EXPORT_UPTIME_INVALID: {
          const result = processUptimeExport(internalExportType, config);
          if (result) {
  ...
  /**
   * [PINDAH] Mengendalikan semua permintaan ekspor dari menu interaktif.
   */
  function handleExportRequest(update, config, userData) {
    const callbackQuery = update.callback_query;
    const chatId = callbackQuery.message.chat.id;
    ...
    try {
      ...
      const sentMessage = kirimPesanTelegram(`⏳ Memulai proses ekspor untuk <b>${titleForStatus}</b>...`, config, "HTML", null, chatId);
    ...
    try {
      ...
      switch (exportType) {
        ...
        case KONSTANTA.CALLBACK.EXPORT_LOG_30_DAYS: {
          const now = new Date();
          let startDate = new Date();
          if (exportType === KONSTANTA.CALLBACK.EXPORT_LOG_TODAY) { startDate.setHours(0, 0, 0, 0); title = "Log Perubahan Hari Ini"; }
          else if (exportType === KONSTANTA.CALLBACK.EXPORT_LOG_7_DAYS) { startDate.setDate(now.getDate() - 7); title = "Log Perubahan 7 Hari Terakhir"; }
          else { startDate.setDate(now.getDate() - 30); title = "Log Perubahan 30 Hari Terakhir"; }
          const combinedLogResult = getCombinedLogs(startDate, config);
          headers = combinedLogResult.headers;
          data = combinedLogResult.data;
          highlightColumn = config[K.HEADER_LOG_ACTION];
      ...
      if (data && headers && headers.length > 0) {
        if (data.length > 0) {
          exportResultsToSheet(headers, data, title, config, userData, highlightColumn);
          if (statusMessageId) editMessageText(`✅ Proses ekspor untuk <b>${title}</b> telah selesai.`, null, chatId, statusMessageId, config);
        } else {
          if (statusMessageId) editMessageText(`ℹ️ Tidak ada data untuk diekspor untuk "<b>${title}</b>".`, null, chatId, statusMessageId, config);
        }
      } else {
        if (statusMessageId) editMessageText(`⚠️ Gagal memproses permintaan ekspor.`, null, chatId, statusMessageId, config);
      }
    } catch (e) {
      handleCentralizedError(e, `Permintaan Ekspor`, config, userData);
      if (statusMessageId) editMessageText(`⚠️ Terjadi kesalahan saat ekspor.\n<code>${escapeHtml(e.message)}</code>`, null, chatId, statusMessageId, config);
    }
  }
