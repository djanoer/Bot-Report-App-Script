...
const MockServices = {
  ...
  CacheService: {
    ...
    removeAll: function(keys) { keys.forEach(key => delete this._cache[key]); },
  ...
  PropertiesService: {
    ...
    deleteProperty: function(key) { delete this._props[key]; },
...
function testAlurCekVmDanDetail() {
  ...
  const mockConfigData = [
      ...
      ["HEADER_VM_HOSTS", "Host"], ["HEADER_VM_TANGGAL_SETUP", "Tanggal Setup"],
  ...
  const mockVmData = [
    ["Primary Key", "Virtual Machine", "IP Address", "State", "Uptime", "CPU", "Memory", "Provisioned (GB)", "Cluster", "Datastore", "Kritikalitas", "Aplikasi BIA", "DEV/OPS", "Guest OS", "vCenter", "No Tiket", "Host", "Tanggal Setup"],
    ["VM-001-VC01", "WEB_SERVER_PROD", "10.10.1.5", "poweredOn", "100", "8", "16", "100", "PROD-CLUSTER-A", "DS_PROD_01", "CRITICAL", "Portal Web", "John Doe", "Linux", "VC01", "TICKET-123", "HOST-01", new Date()]
  ];
  MockServices.SpreadsheetApp.setSheetData("Data VM Uji Coba", mockVmData);

  MockServices.SpreadsheetApp.setSheetData("Hak Akses", [["User ID", "Nama", "Email", "Role"], ['1', 'Tester', 'tester@example.com', 'Admin']]);
  MockServices.SpreadsheetApp.setSheetData("Catatan VM", [["Primary Key", "Isi Catatan", "Timestamp Update", "Nama User Update"]]);
...
function testAlurEksporLengkap() {
  ...
  MockServices.SpreadsheetApp.setSheetData("Konfigurasi", mockConfigData);
  MockServices.SpreadsheetApp.setSheetData("Log Perubahan", [["Timestamp", "Aksi"], [new Date(), "MODIFIKASI"]]);
  ...
  const updatePalsu = {
    callback_query: {
      id: "query123", from: { id: 1, first_name: "Tester" },
